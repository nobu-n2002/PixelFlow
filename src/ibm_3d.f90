program main
  !$ use omp_lib
   implicit none
   integer,parameter:: md = 320, nd = 320, ld = 320   ! md, nd, ld > grid size (m,n,l)
   real:: dx, dy, dz, dt
   real:: xnue, xlamda, density, width, height, depth, time
   real:: inlet_velocity, outlet_pressure, AoA, thickness
   real,dimension(0:md,0:nd,0:ld):: u, v, w, p, u_old, v_old, w_old, p_old
   ...
   real:: residual_p, residual_u, residual_v, residual_w 
  ...
  do istep = 1, istep_max
    ...
    do i = 0, m+1
      do j = 0, n+1
        do k = 0, l+1
          residual_p = max(residual_p, porosity(i,j,k) * abs(p(i,j,k) - p_old(i,j,k)))
  ...
  subroutine  solve_p (p, u, v, w, u_old, v_old, w_old, porosity, &
   ...
   implicit none
   integer,parameter:: md = 320, nd = 320, ld = 320
  ...
  !$omp & shared(p_old, p, ap, ae, aw, an, as, at, ab, bb) &
  !$omp & default(none)
  ...
  subroutine  solve_matrix_vec_oacc (p, ap, ae, aw, an, as, at, ab, bb, m, n, l)
   implicit none
   integer,parameter:: md=320, nd = 320, ld = 320     ! md, nd > grid size (m,n)
   real,intent(inout),dimension(0:md,0:nd,0:ld):: p
   real,intent(in),dimension(0:md,0:nd,0:ld):: ap, ae, aw, an, as, at, ab, bb
   integer,intent(in):: m, n, l
  ...
  open (320, file='etc/grid.dat', status='replace')
  ! ----------------
  write(320,*)'m, n, l =', m, n, l
  write(320,*)'grid points ='
  write(320,*) (xp(i), i=1,m)
  write(320,*) (yp(j), j=1,n)
  write(320,*) (zp(k), k=1,l)
  ! ----------------
  close (320)
  return
  ...
  open (320, file='etc/cellcenter.dat', status='replace')
  ! ----------------
